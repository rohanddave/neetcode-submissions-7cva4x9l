...
class Solution:
    def isBalanced(self, root: Optional[TreeNode]) -> bool:
        def bottom_up_postorder_dfs2(node): 
            if not node: 
                return 0
            
            l_height = bottom_up_postorder_dfs2(node.left)
            if l_height == -1:
                return -1
                
            r_height = bottom_up_postorder_dfs2(node.right)
            if r_height == -1:
                return -1
        ...
        return bottom_up_postorder_dfs2(root) != -1
